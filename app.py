...
try:
    ...
    rf_model = None   # SHAP section will be skipped if file is missing

# ── Transaction dataset loaded lazily inside generate_graph_chart() ──────────
# (file does not exist on first deploy — created only after first transaction)
...
def generate_shap_chart(feature_values_dict):
    ...
    """
    if rf_model is None:
        return None   # rf_model.pkl not uploaded yet
...
def generate_graph_chart(location_val, feature_values_dict):
    ...
    """
    # Load fresh each call — file is created only after the first transaction
    if not os.path.isfile('dynamic_transactions.csv'):
        return None   # no data yet, skip silently
