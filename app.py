...
# 0.50 gives correct fraud detection while keeping legit transactions safe.
...
# The scaler was trained on PaySim data where the mean transaction amount is
# ~₹2,85,000. This makes realistic UPI amounts (₹500–₹1,00,000) look tiny and
# below-average to the model, so amount alone never triggers fraud.
...
def amount_risk_boost(amount):
    """
    Returns a small additive boost (0.0 to 0.12) based on transaction amount.
    Thresholds are calibrated to realistic Indian UPI transaction ranges.
    This supplements the XGBoost score which under-weights amount due to
    the PaySim training data scale mismatch.
    """
    if amount >= 100000:   # ₹1 lakh+  → high risk
        return 0.12
    elif amount >= 50000:  # ₹50k–1L   → moderate-high risk
        return 0.08
    elif amount >= 25000:  # ₹25k–50k  → moderate risk
        return 0.04
    elif amount >= 10000:  # ₹10k–25k  → slight risk
        return 0.02
    else:
        return 0.0
...
def generate_graph_chart(location_val, feature_values_dict, prediction):
        ...
        try:
            ...
            if all(c in df_full.columns for c in required) and len(df_full) >= 3:
                ...
                if not df_loc.empty:
                    ...
                    if THIS_USER not in G:
                        G.add_node(THIS_USER)
                        if len(users_at_loc) > 0:
                            G.add_edge(THIS_USER, users_at_loc[0])
    ...
    # Synthetic illustrative graph
    np.random.seed(42)
    normal_ids   = np.random.randint(1000, 4999, size=12)
    fraud_ids    = np.random.randint(6000, 7999, size=4)
    highrisk_ids = np.random.randint(8000, 8999, size=3)
    ...
    for nid in np.random.choice(normal_ids, size=4, replace=False):
...
def predict():
    try:
        ...
        # FIX 2: Add amount-based risk boost to compensate for PaySim scale mismatch.
        # The training data mean amount (~₹2.85L) makes all realistic UPI amounts
        # look below-average. This boost restores amount as a proper risk signal.
        boost        = amount_risk_boost(amount)
        hybrid_score = min(1.0, 0.70 * xgb_prob + 0.30 * mse_norm + boost)

        # FIX 1: Use recalibrated threshold (0.50) instead of saved 0.69
